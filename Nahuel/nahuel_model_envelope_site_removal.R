...
## ------------------------------------------------------------
## Script: nahuel_model_envelope_site_removal.r
## ------------------------------------------------------------
...
for(m in seq_len(n_model_reps)){
  ...
  message("Running Nahuel model replicate ", m, " / ", n_model_reps)
...
write.csv(empirical_results,
          file.path(out_dir, "nahuel_empirical_divergence_by_subset.csv"),
          row.names = FALSE)

write.csv(model_results,
          file.path(out_dir, "nahuel_model_generated_divergence_by_subset.csv"),
          row.names = FALSE)

write.csv(comparison_summary,
          file.path(out_dir, "nahuel_empirical_vs_model_envelope_summary.csv"),
          row.names = FALSE)
...
p <- ggplot() +
  ...
  ggtitle(
    "Nahuel: empirical divergence vs model-generated envelope",
    subtitle = "Ribbon = 95% envelope of model-generated mean curves; solid line = empirical"
  )

ggsave(file.path(out_dir, "nahuel_empirical_vs_model_envelope.png"),
       p,
       width = 8,
       height = 5.5,
       dpi = 300)
