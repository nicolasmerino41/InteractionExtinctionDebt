...
## ------------------------------------------------------------
## Script: nahuel_mechanism1_interaction_repeatability.r
## ------------------------------------------------------------
...
out_dir <- "Nahuel/mechanism1"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

webs <- read_nahuel_webs()
all_sites <- seq_along(webs)

site_tables <- build_nahuel_site_tables(webs, all_sites)
...
write.csv(empirical_repeatability,
          file = file.path(out_dir, "nahuel_empirical_interaction_repeatability.csv"),
          row.names = FALSE)
...
for(r in seq_len(n_model_reps)){
  ...
  message("Nahuel model replicate ", r, " / ", n_model_reps)
...
write.csv(model_repeatability,
          file = file.path(out_dir, "nahuel_model_generated_interaction_repeatability.csv"),
          row.names = FALSE)
...
write.csv(summary_by_source,
          file = file.path(out_dir, "nahuel_repeatability_summary_by_source.csv"),
          row.names = FALSE)
...
write.csv(model_summary_by_rep,
          file = file.path(out_dir, "nahuel_model_summary_by_rep.csv"),
          row.names = FALSE)

write.csv(comparison_to_model,
          file = file.path(out_dir, "nahuel_empirical_vs_model_repeatability_summary.csv"),
          row.names = FALSE)
...
p_hist <- ggplot(plot_data,
  ...
  ggtitle("Nahuel: spatial repetition of realised interactions",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "nahuel_hist_interacting_sites_empirical_vs_model.png"),
       p_hist,
       width = 8,
       height = 5.5,
       dpi = 300)
...
p_density <- ggplot(plot_data,
  ...
  ylab("Density") +
  ggtitle("Nahuel: interaction repeatability",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "nahuel_density_repeatability_empirical_vs_model.png"),
       p_density,
       width = 8,
       height = 5.5,
       dpi = 300)
...
p_box <- ggplot(plot_data,
  ...
  ggtitle("Nahuel: interaction spatial repetition",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "nahuel_boxplot_interacting_sites_empirical_vs_model.png"),
       p_box,
       width = 7,
       height = 5.5,
       dpi = 300)
