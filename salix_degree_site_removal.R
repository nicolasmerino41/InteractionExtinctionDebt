...
## ------------------------------------------------------------
## Script: salix_degree_site_removal.R
## Network: Salix Galpar, i.e. galler-parasitoid
## ------------------------------------------------------------
dataset_label <- "Salix"
out_dir <- "Salix/degree_site_removal"
p_fixed <- 0.121
load_dataset_webs <- function(){
  ...
  site_interact <- as.data.frame(site_interact)
  site_interact <- site_interact %>%
    dplyr::filter(RPAR != "none", !is.na(SITE), !is.na(RPAR), !is.na(RGALLER), SITE != "", RPAR != "", RGALLER != "")
  webs <- list()
  for(s in sort(unique(site_interact$SITE))){
    cur <- site_interact %>% dplyr::filter(SITE == s)
    mat <- xtabs(NB_GALLS_PAR ~ RGALLER + RPAR, data = cur)
    mat <- as.matrix(mat)
    mat[mat != 0] <- 1
    webs[[as.character(s)]] <- mat
  }
  webs
}
