...
## ------------------------------------------------------------
## Script: quercus_degree_shape_followup.R
## ------------------------------------------------------------

dataset_label <- "Quercus"
p_fixed <- 0.0227
previous_degree_dir <- "Quercus/degree_site_removal"
out_dir <- "Quercus/degree_shape_followup"

load_dataset_webs <- function(){
  if(!requireNamespace("XLConnect", quietly = TRUE)) stop("XLConnect is required for Quercus. Install binary if possible: install.packages('XLConnect', type = 'binary')")
  ...
  for(t in metadata$Tree){
    ...
    int_mat <- int_mat[-1]
    if(length(which(colnames(int_mat) %in% c("Unpar", "unpar"))) > 0) int_mat <- int_mat[-which(colnames(int_mat) %in% c("Unpar", "unpar"))]
    int_mat <- as.matrix(int_mat); suppressWarnings(storage.mode(int_mat) <- "numeric")
    int_mat[is.na(int_mat)] <- 0; int_mat[int_mat != 0] <- 1
    r_remove <- which(rowSums(int_mat) == 0); if(length(r_remove) != 0) int_mat <- int_mat[-r_remove, , drop = FALSE]
    c_remove <- which(colSums(int_mat) == 0); if(length(c_remove) != 0) int_mat <- int_mat[, -c_remove, drop = FALSE]
    webs[[as.character(t)]] <- int_mat
  }
  webs
}
