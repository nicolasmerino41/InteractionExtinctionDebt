## ------------------------------------------------------------
## Script: gottin_PP_model_envelope_site_removal.R
## ------------------------------------------------------------
...
p_fixed <- 0.044  # CHANGE if you have the calibrated Gottin-PP value
...
dir.create("Gottin/envelope", recursive = TRUE, showWarnings = FALSE)
  ...
  for(i in seq_len(nrow(subset_object$index))){
    ...
    out[[i]] <- data.frame(
      ...
      relative_divergence_raw =
        safe_relative_difference(observed_links, exp_row$expected_links_raw),
      divergence_conditioned = observed_links - exp_row$expected_links_conditioned,
      relative_divergence_conditioned =
        safe_relative_difference(observed_links, exp_row$expected_links_conditioned)
    )
  }
...
write.csv(empirical_results,
          paste0("Gottin/envelope/", output_prefix,
                 "_empirical_divergence_by_subset.csv"),
          row.names = FALSE)

write.csv(model_results,
          paste0("Gottin/envelope/", output_prefix,
                 "_model_generated_divergence_by_subset.csv"),
          row.names = FALSE)

write.csv(comparison_summary,
          paste0("Gottin/envelope/", output_prefix,
                 "_empirical_vs_model_envelope_summary.csv"),
          row.names = FALSE)
...
p <- ggplot() +
  ...
  ggtitle(
    "Gottin-PP: empirical divergence vs model-generated envelope",
    subtitle = "Ribbon = 95% envelope of model-generated mean curves; solid line = empirical"
  )

ggsave(paste0("Gottin/envelope/", output_prefix,
              "_empirical_vs_model_envelope.png"),
       p,
       width = 8,
       height = 5.5,
       dpi = 300)
