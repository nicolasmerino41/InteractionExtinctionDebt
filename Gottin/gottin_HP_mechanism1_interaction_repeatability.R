## ------------------------------------------------------------
## Script: gottin_HP_mechanism1_interaction_repeatability.R
##
## Purpose:
## Test whether empirical realised interactions are more spatially
## repeated than model-generated interactions in Gottin-HP.
##
## Outputs saved in:
##   Gottin/mechanism1/
##
...
## ---------------------------
## 5. Build empirical co-occurrence and interaction tables
## ---------------------------
...
## ---------------------------
## 7. Empirical interaction repeatability
## ---------------------------
...
write.csv(empirical_repeatability,
          file = file.path(out_dir, "gottin_HP_empirical_interaction_repeatability.csv"),
          row.names = FALSE)


## ---------------------------
## 8. Model-generated interaction repeatability
## ---------------------------
...
for(r in seq_len(n_model_reps)){
  ...
  message("Gottin-HP model replicate ", r, " / ", n_model_reps)
...
write.csv(model_repeatability,
          file = file.path(out_dir, "gottin_HP_model_generated_interaction_repeatability.csv"),
          row.names = FALSE)
...
write.csv(summary_by_source,
          file = file.path(out_dir, "gottin_HP_repeatability_summary_by_source.csv"),
          row.names = FALSE)
...
write.csv(model_summary_by_rep,
          file = file.path(out_dir, "gottin_HP_model_summary_by_rep.csv"),
          row.names = FALSE)

write.csv(comparison_to_model,
          file = file.path(out_dir, "gottin_HP_empirical_vs_model_repeatability_summary.csv"),
          row.names = FALSE)
...
p_hist <- ggplot(plot_data,
  ...
  ggtitle("Gottin-HP: spatial repetition of realised interactions",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "gottin_HP_hist_interacting_sites_empirical_vs_model.png"),
       p_hist,
       width = 8,
       height = 5.5,
       dpi = 300)
...
p_density <- ggplot(plot_data,
  ...
  ylab("Density") +
  ggtitle("Gottin-HP: interaction repeatability",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "gottin_HP_density_repeatability_empirical_vs_model.png"),
       p_density,
       width = 8,
       height = 5.5,
       dpi = 300)
...
p_box <- ggplot(plot_data,
  ...
  ggtitle("Gottin-HP: interaction spatial repetition",
          subtitle = "Empirical vs model-generated links")

ggsave(file.path(out_dir, "gottin_HP_boxplot_interacting_sites_empirical_vs_model.png"),
       p_box,
       width = 7,
       height = 5.5,
       dpi = 300)
