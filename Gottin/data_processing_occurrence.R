...
## network data
ant_raw <- read.csv("Gottin/raw-data/host_para_all_interactions.csv", head=T, sep=";")
mut_raw <- read.csv("Gottin/raw-data/plant_poll_all_interactions.csv", head=T, sep=";")

## metadata environmental variables (e.g., size of grassland fragments)
metadata <- read.csv("Gottin/metadata.csv", head=T, sep=",")
