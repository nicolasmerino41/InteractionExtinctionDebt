## ------------------------------------------------------------
## Script: gottin_PP_controlled_n_occ_sites.R
##
## Question:
...
write.csv(empirical_pairs,
          file.path(out_dir, "gottin_PP_empirical_pair_repeatability.csv"),
          row.names = FALSE)
...
for(r in seq_len(n_model_reps)){
  ...
  message("Gottin-PP controlled simulation ", r, " / ", n_model_reps)
...
write.csv(model_pairs,
          file.path(out_dir, "gottin_PP_model_generated_pair_repeatability.csv"),
          row.names = FALSE)
...
write.csv(comparison_exact,
          file.path(out_dir, "gottin_PP_controlled_exact_n_cooccurring_sites.csv"),
          row.names = FALSE)

write.csv(comparison_binned,
          file.path(out_dir, "gottin_PP_controlled_binned_n_cooccurring_sites.csv"),
          row.names = FALSE)
...
p_exact_repeatability <- ggplot(plot_exact_data,
  ...
  ggtitle("Gottin-PP: repeatability controlled for co-occurring sites",
          subtitle = "Ribbon = 95% model envelope; points/solid line = empirical")

ggsave(file.path(out_dir, "gottin_PP_exact_controlled_mean_repeatability.png"),
...
p_exact_interacting <- ggplot(plot_exact_data,
  ...
  ggtitle("Gottin-PP: interacting sites controlled for co-occurring sites",
          subtitle = "Ribbon = 95% model envelope; points/solid line = empirical")

ggsave(file.path(out_dir, "gottin_PP_exact_controlled_mean_interacting_sites.png"),
       p_exact_interacting,
       width = 8,
       height = 5.5,
       dpi = 300)
...
p_binned_repeatability <- ggplot(comparison_binned,
  ...
  ylab("Mean repeatability") +
  ggtitle("Gottin-PP: binned control for co-occurring sites",
          subtitle = "Error bars = 95% model envelope; filled points = empirical")

ggsave(file.path(out_dir, "gottin_PP_binned_controlled_mean_repeatability.png"),
