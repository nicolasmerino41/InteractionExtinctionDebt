...
metadata <- read.csv('Quercus/metadata.csv',  stringsAsFactors = F)
...
for(t in metadata$Tree){
  excel <- loadWorkbook(paste0("Quercus/raw-data/Web",t," 2006 Kaartinen.xlsx"))
  # get sheet names
...
for(con in unique(real_interactions$consumer)){
  ...
  for(res in unique(cur_sp$resource)){
    ...
    z <- (consumer_sites %in% resource_sites)
    number_cooc <- length(z[z == TRUE])
    for(p in unique(intersect(consumer_sites,resource_sites))){ 
      excel <- loadWorkbook(paste0("./raw-data/Web",p," 2006 Kaartinen.xlsx"))
      # get sheet names
      sheet_names <- getSheets(excel)
      names(sheet_names) <- sheet_names
      sheet_list <- lapply(sheet_names, function(.sheet){readWorksheet(object=excel, .sheet)})
      ...
      local_com <- igraph::graph_from_incidence_matrix(int_mat, directed = T, mode='out')
...
# We calculate the proportion of sites in which every pair of species co-occur from the consumer perspective
...
write.csv(realisations_indegree, file = 'random_realisations_expected_indegree_quercus.csv')
write.csv(realisations_outdegree, file = 'random_realisations_expected_outdegree_quercus.csv')
...
expected_int_model_indegree <- expected_int_model %>%
  dplyr::group_by(species) %>%
  dplyr::mutate(expected_indegree = sum(prob_expected))
...
write.csv(expected_realised_indegree_freq, file = 'expected_realised_indegree_quercus.csv')
...
write.csv(expected_realised_outdegree_freq, file = 'expected_realised_outdegree_quercus.csv')
...
write.csv(occupancy_pred, file = 'occupancy_pred_quercus.csv')
write.csv(occupancy_prey, file = 'occupancy_prey_quercus.csv')
...
write.csv(network_real_pred, file = 'network_real_pred_quercus.csv')
write.csv(network_real_prey, file = 'network_real_prey_quercus.csv')
write.csv(network_coocurrence_pred, file = 'network_coocurrence_pred_quercus.csv')
write.csv(network_coocurrence_prey, file = 'network_coocurrence_prey_quercus.csv')
...
p <- 0.11 # o here is the percentage of interactions that where realised from the co-occurrence network to the network of biotic interactions
...
write.csv(realisations_indegree_null_model, file = 'null_random_realisations_expected_indegree_quercus.csv')
write.csv(realisations_outdegree_null_model, file = 'null_random_realisations_expected_outdegree_quercus.csv')
