...
## ------------------------------------------------------------
## Script: data_processing_occurrence_quercus_site_removal.r
##
## Purpose:
## Signed site-removal divergence exploration for the Quercus dataset.
## ------------------------------------------------------------

set.seed(123)

p_fixed <- 0.0227
output_prefix <- "quercus"

webs <- read_quercus_webs()
all_sites <- names(webs)

run_quercus_subset <- function(webs,
                              sites_keep,
                              p_fixed,
                              removal_fraction = NA_real_,
                              rep_id = NA_integer_){

  site_tables <- build_quercus_site_tables(webs, sites_keep)
...
for(removal in removal_levels){
  ...
  for(r in seq_len(reps_here)){
    ...
    results_list[[counter]] <- run_quercus_subset(
      webs = webs,
...
write.csv(site_removal_results,
          file = "Quercus/quercus_site_removal_divergence.csv",
          row.names = FALSE)
...
plot_raw <- ggplot(site_removal_results,
  ...
  ggtitle("Quercus: signed divergence under random site removal",
          subtitle = "Expected links calculated with raw pairwise interaction probabilities")

ggsave("Quercus/quercus_site_removal_relative_divergence_raw.png",
       plot_raw,
       width = 7,
       height = 5,
       dpi = 300)
...
ggsave("Quercus/quercus_site_removal_signed_divergence_raw.png",
       plot_signed,
       width = 7,
       height = 5,
       dpi = 300)
...
write.csv(summary_table,
          file = "Quercus/quercus_site_removal_summary.csv",
          row.names = FALSE)
