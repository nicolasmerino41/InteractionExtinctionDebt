...
pairwise_set_dissimilarity <- function(df, sites_keep, max_site_pairs = 5000){
  ...
  site_pairs <- utils::combn(seq_len(n_sites), 2)
...
run_one_dataset <- function(dataset){
  ...
  by_subset_list <- future.apply::future_lapply(
    seq_len(nrow(subset_object$index)),
    function(i){
  ...
  by_subset <- bind_rows(by_subset_list)
  summary <- by_subset %>%
...
interaction_jaccard_limits <- range(
  combined_summary$interaction_mean_jaccard,
  na.rm = TRUE
)

cooccurrence_jaccard_limits <- range(
  combined_summary$cooccurrence_mean_jaccard,
  na.rm = TRUE
)

plot_beta <- function(metric, ylab, filename, y_limits){
  
  p <- ggplot(combined_summary,
              aes(removal_fraction, .data[[metric]])) +
  ...
  ggsave(
    file.path(combined_out, filename),
    p,
    width = 14,
    height = 7,
    dpi = 300
...
plot_beta(
  "interaction_mean_jaccard",
  "Mean interaction Jaccard dissimilarity",
  "09_interaction_beta_diversity_vs_site_removal.png",
  interaction_jaccard_limits
)

plot_beta(
  "cooccurrence_mean_jaccard",
  "Mean co-occurrence Jaccard dissimilarity",
  "09_cooccurrence_beta_diversity_vs_site_removal.png",
  cooccurrence_jaccard_limits
)
