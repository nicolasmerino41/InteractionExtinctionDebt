...
model_specs <- data.frame(
  model_type = c(
    ...
    "M2_resource",
    "M3_site",
    "M4_consumer_resource",
    "M5_consumer_resource_site",
    "M6_pair",
    "M7_pair_site"
  ),
  formula_text = c(
    ...
    "interaction_observed ~ consumer + resource",
    "interaction_observed ~ consumer + resource + site",
    "interaction_observed ~ pair_id",
    "interaction_observed ~ pair_id + site"
  ),
  is_pair_model = c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE, TRUE),
...
run_one_dataset <- function(dataset){
  ...
  write.csv2(
    fit_summary,
    file.path(out_dir, paste0(dataset, "_script13_site_level_model_fit_summary_long.csv")),
    row.names = FALSE
  )

  write.csv2(
    cv_summary,
    file.path(out_dir, paste0(dataset, "_script13_site_level_model_cv_summary_long.csv")),
    row.names = FALSE
  )

  write.csv2(
    retained_summary,
    file.path(out_dir, paste0(dataset, "_script13_retained_link_prediction_summary_long.csv")),
    row.names = FALSE
  )
...
write.csv2(
  fit_summary_all,
  file.path(combined_out, "script13_site_level_model_fit_summary_combined_long.csv"),
  row.names = FALSE
)

write.csv2(
  cv_summary_all,
  file.path(combined_out, "script13_site_level_model_cv_summary_combined_long.csv"),
  row.names = FALSE
)

write.csv2(
  retained_prediction_all,
  file.path(combined_out, "script13_retained_link_prediction_by_subset_combined_long.csv"),
  row.names = FALSE
)

write.csv2(
  retained_summary_all,
  file.path(combined_out, "script13_retained_link_prediction_summary_combined_long.csv"),
  row.names = FALSE
)
...
ggsave(
  file.path(combined_out, "13_model_fit_improvement_relative_to_M0_long.png"),
  p_fit,
  width = 14,
  height = 7,
  dpi = 300
...
ggsave(
  file.path(combined_out, "13_retained_link_prediction_error_by_model_long.png"),
  p_error,
  width = 14,
  height = 7,
  dpi = 300
...
ggsave(
  file.path(combined_out, "13_observed_vs_expected_retained_links_by_model_long.png"),
  p_obs_exp,
  width = 14,
  height = 7,
  dpi = 300
...
write.csv2(
  ...
  file.path(combined_out, "script13_component_importance_summary_long.csv"),
  row.names = FALSE
)
...
ggsave(
  file.path(combined_out, "13_component_importance_summary_long.png"),
  p_component,
  width = 14,
  height = 7,
  dpi = 300
